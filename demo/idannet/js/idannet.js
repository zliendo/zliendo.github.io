...
$(document).ready(function(){
	...
	$.getJSON(patient_info_url, function( patient_data ) {
		...
		if  (idannet_response_url.indexOf("predict_esi") >=0) {
		      patient_info_JSON = JSON.stringify(patient_data);
		     // call to IDANN rest api with patient_info appended 
			// console.log(patient_info);
			 //console.log(patient_info_JSON);
			 console.log ('API service call');

			// jsonp_url = idannet_response_url + "?callback=?"

			//$.getJSON( jsonp_url, patient_info_JSON, function( data ) {
			//$.post( idannet_response_url,patient_info, function( data ) {
			
			/*$.post( idannet_response_url,patient_info_JSON, function( data ) {
         ...
         error:function(jqXHR,textStatus,errorThrown)
         {
			...
			$.ajax({
				url : idannet_response_url ,
				type : "POST",
				dataType :"json",
				data : patient_info_JSON,
				 contentType: "application/json; charset=utf-8",
				success : function(response) {

					idannet_response = response;	
					render_idann_viz(idannet_response, patient_info);	
					
				},
				error : function(xhr, status, error) {
					console.log("Received error response for requesting idann esi: ",
							xhr.responseText + " " + status + " " + error);
					//displayNodeValidateResultModal(error);
					// console.log(data);
				},
